class HostsController < ApplicationController
  ...
  def index
    @hosts = Host.all
    @hosts = @hosts.where(["first_name LIKE :filter OR last_name LIKE :filter", :filter => "%" + params[:filter] + "%"]) unless params[:filter].blank?
    @hosts = @hosts.order((params[:by] || "first_name") + " " + (params[:order] || "asc"))
    @hosts = @hosts.page(params[:page] || 1)
  end
  ...
  private
    ...
    def host_params
      params.require(:host).permit(:first_name, :last_name, :sex, :age)
    end
end
